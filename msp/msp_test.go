...
func TestMSPSetupNoCryptoConf(t *testing.T) {
    ...
    conf, err := GetLocalMspConfig("DEFAULT", mspDir, nil)
    if err != nil {
...
func TestMSPSetupBad(t *testing.T) {
    _, err := GetLocalMspConfig("DEFAULT", "barf", nil)
    if err == nil {
...
func TestNotFoundInBCCSP(t *testing.T) {
    dir := GetDevMspDir()
    conf, err := GetLocalMspConfig("DEFAULT", dir, nil)
...
func TestBadAdminIdentity(t *testing.T) {
    conf, err := GetLocalMspConfig("DEFAULT", "testdata/badadmin", nil)
    assert.NoError(t, err)
...
func TestGetOU(t *testing.T) {
    ...
    units := id.GetOrganizationalUnits()
    if units == nil {
        t.Fatal("units nil")
        return
    }
...
func TestMain(m *testing.M) {
    ...
    if id == "" {
        fmt.Println("Failed obtaining identifier id for localMSP")
        os.Exit(-1)
    }
